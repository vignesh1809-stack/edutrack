...
import { setDashboardFilter, fetchYearsRequest } from '../store/actions/dashboardActions';

const FilterSection = () => {
  const dispatch = useDispatch();
  const { filters, availableYears } = useSelector((state) => state.dashboard);
  const { year: selectedYear } = filters;
  
  const [activeDropdown, setActiveDropdown] = useState(null); // 'year' | null
          ...
          >
            {availableYears.map((year) => (
              <button 
                key={year}
                onClick={() => {
                  dispatch(setDashboardFilter({ year }));
