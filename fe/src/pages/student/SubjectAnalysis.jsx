...
import { fetchStudentSubmissionDetailsRequest } from '../../store/actions/studentDashboardActions';
...
const SubjectAnalysis = () => {
    ...
    const { loading, data: analysisData, error } = useSelector(
        state => state.studentDashboard.subjectAnalysis || { loading: false, data: null, error: null }
    );

    useEffect(() => {
        if (submissionId) {
            dispatch(fetchStudentSubmissionDetailsRequest(submissionId));
        }
    }, [dispatch, submissionId]);

    // Fallback to static mock values if no dynamic data loaded
    const courseName = analysisData?.courseName || 'Advanced Mathematics';
    const subTitle = analysisData?.examType ? `${analysisData.examType} Analysis` : 'Advanced Mathematics Analysis';
    const examType = analysisData?.examType || 'Mid-Term Examination';
    const feedbackStatus = analysisData?.feedbackStatus || 'High Distinction';
    const score = analysisData ? `${analysisData.score} / ${analysisData.maxScore}` : '92 / 100';
    const grade = analysisData?.grade || 'A+';
    const classAverage = analysisData ? `${analysisData.classAverage} / ${analysisData.maxScore}` : '74 / 100';
    const overallFeedback = analysisData?.overallFeedback || 'Detailed analysis for Mid-Term Examination. Your performance places you in the top 5% of the global cohort.';
        ...
        <div className="bg-surface text-on-surface antialiased mb-24 md:mb-0 font-body min-h-screen">
            ...
            <div className="md:pl-64">
                ...
                <header className="fixed top-0 w-full z-50 bg-surface/70 backdrop-blur-md shadow-sm flex items-center justify-between px-6 h-16 md:w-[calc(100%-16rem)]">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="hidden md:block font-label text-on-surface-variant text-sm font-medium">{subTitle}</span>
                ...
                <main className="pt-24 pb-32 px-6 max-w-6xl mx-auto space-y-8">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20 space-y-4">
                            ...
                            <p className="text-on-surface-variant font-medium text-sm">Loading deep-dive AI evaluation report...</p>
                        </div>
                    ) : error ? (
                        <div className="bg-error-container text-on-error-container p-6 rounded-3xl space-y-3">
                            <h4 className="font-bold font-headline text-lg">Failed to Load Report</h4>
                            <p className="text-sm leading-relaxed">{error}</p>
                            <button 
                                onClick={() => window.location.reload()}
                                className="bg-on-error-container text-error-container px-4 py-2 rounded-xl text-xs font-bold hover:opacity-90 active:scale-95 transition-all"
                            >
                                Try Again
                            </button>
                        </div>
                            ...
                            <section className="grid grid-cols-1 gap-6">
                                <div className="bg-surface-container-lowest rounded-3xl p-8 flex flex-col justify-between relative overflow-hidden shadow-sm border border-surface-container">
                                    <div className="space-y-2 max-w-md">
                                        <span className="bg-tertiary-container text-on-tertiary-container px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider inline-block">{feedbackStatus}</span>
                                        <h2 className="text-4xl font-extrabold tracking-tight font-headline text-on-surface">{courseName}</h2>
                                        <p className="text-on-surface-variant text-base leading-relaxed mt-2">{overallFeedback}</p>
                                    </div>
                                    <div className="mt-8 flex gap-8">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-2xl font-bold text-on-surface">{classAverage}</p>
                                        </div>
                                    </div>
                                </div>
                            </section>
