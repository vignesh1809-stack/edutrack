...
public interface DepartmentRepository extends JpaRepository<Department, UUID> {
    ...
    @Query(value = "SELECT DISTINCT section FROM classes WHERE institution_id = UNHEX(REPLACE(:institutionId, '-', '')) AND batch_year = :batchYear AND is_deleted = 0 AND section IS NOT NULL ORDER BY section ASC", nativeQuery = true)
    List<String> findDistinctSectionsByBatchYear(@Param("institutionId") String institutionId, @Param("batchYear") int batchYear);

    @Query(value = "SELECT DISTINCT section FROM classes WHERE institution_id = UNHEX(REPLACE(:institutionId, '-', '')) AND is_deleted = 0 AND section IS NOT NULL ORDER BY section ASC", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT name FROM departments WHERE institution_id = UNHEX(REPLACE(:institutionId, '-', '')) AND is_deleted = 0 ORDER BY name ASC", nativeQuery = true)
    List<String> findDistinctNames(@Param("institutionId") String institutionId);
}
